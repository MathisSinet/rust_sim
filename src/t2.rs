...
impl T2vars {
    fn init() -> Self {
        T2vars {
            dq1: Variable::new(
                FirstFreeCost { model: ExponentialCost::new(10., 2.) },
                StepwiseValue::new(2., 10)
            ),
            dq2: Variable::new(
                ExponentialCost::new(5e3, 2.),
                StepwiseValue::new(2.,10)
            ),
            dq3: Variable::new(
                ExponentialCost::new(3e25,3.),
                StepwiseValue::new(2.,10)
            ),
            dq4: Variable::new(
                ExponentialCost::new(8e50, 4.),
                StepwiseValue::new(2.,10)
            ),
            dr1: Variable::new(
                ExponentialCost::new(2e6, 2.),
                StepwiseValue::new(2.,10)
            ),
            dr2: Variable::new(
                ExponentialCost::new(3e9, 2.),
                StepwiseValue::new(2.,10)
            ),
            dr3: Variable::new(
                ExponentialCost::new(4e25, 3.),
                StepwiseValue::new(2., 10)
            ),
            dr4: Variable::new(
                ExponentialCost::new(5e50, 4.),
                StepwiseValue::new(2., 10)
            )
        }
    }
...
impl T2 {
    ...
    fn eval_ratio(&self, id:usize) -> BuyEval {
        ...
        let mut bestind = 0;
        for i in 1..8 {
            if costs[i] < costs[bestind] { bestind = i; }
        }

        if bestind == id { BuyEval::BUY } else { BuyEval::SKIP }
    }
    ...
    fn tick(&mut self){
        ...
        self.rho = log10add(self.rho, self.layers[0]*1.15 + self.layers[4]*1.15 + self.multiplier + logdt);
    ...
    fn buy(&mut self) {
        ...
        for i in 0..8 {
            ...
            if self.vars.get(ids[i]).get_level() >= self.t2data.caps[ids[i]] { continue; }
